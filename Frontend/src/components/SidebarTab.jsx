...
import { Menu, X, ChevronLeft, ChevronDown, Layers, Shield, FileText, ListChecks, PlusCircle, Users, History, Building2, User as UserIcon } from 'lucide-react';
...
const SidebarTab = ({ open, setOpen, navItems, isLoaded, user, isAdmin }) => {
  ...
  const baseItem = 'group relative flex items-center gap-3 rounded-md h-10 px-3 text-sm font-medium outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/50 transition-colors';
  ...
  const groups = useMemo(() => {
    ...
    const g = [
      {
        ...
        label: 'Reports',
        icon: FileText,
        items: [find('/report/new'), find('/user/reports'), isAdmin ? find('/admin/reports') : null].filter(Boolean)
      },
      {
        key: 'services',
        label: 'Services',
    ...
    if (isAdmin) {
      g.push({
        ...
        icon: Shield,
        items: [find('/admin'), find('/users'), find('/audit-logs')].filter(Boolean)
      });
    }
    return g.filter(gr => gr.items.length);
  }, [navItems, isAdmin]);

  const pathIsActive = (paths) => paths.some(p => location.pathname === p || location.pathname.startsWith(p + '/'));
  const initiallyOpen = groups.reduce((acc, g) => { acc[g.key] = pathIsActive(g.items.map(i => i.to)); return acc; }, {});
  ...
  <div className="flex items-center h-14 px-3 border-b border-default bg-surface">
        {open ? (
          <>
            ...
            >
              <img src={logo} alt="Logo" className="h-7 w-7 rounded-sm object-contain ring-1 ring-default hover:ring-primary transition" />
      ...
      {/* Navigation (grouped) */}
  <nav className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300/60 dark:scrollbar-thumb-gray-600/60 scrollbar-track-transparent py-4 px-2" aria-label="Primary">
          ...
          {groups.map(group => {
            ...
            return (
              <li key={group.key} className="relative">
                ...
                >
                  <span className={`inline-flex items-center justify-center shrink-0 h-7 w-7 rounded-md transition-colors ${open ? 'bg-primary/15 text-primary group-hover:bg-primary/20' : 'text-primary/80'} ${groupActive ? 'bg-primary text-white' : ''}`}>
                    <GroupIcon size={16} />
                  </span>
                  {open && <span className="truncate flex-1 text-left">{group.label}</span>}
                  {open && <ChevronDown size={14} className={`transition-transform ${expanded ? 'rotate-180' : ''}`} />}
                  {!open && (
                ...
                {expanded && open && (
                  <ul id={`group-${group.key}`} className="mt-1 ml-2 flex flex-col gap-1 border-l border-default pl-2">
                      ...
                      return (
                        <li key={item.to} className="relative">
                          <NavLink
                            ...
                            className={({ isActive }) => `${baseItem} h-9 pl-2 pr-2 ${accentBar} ${isActive ? 'active bg-primary/20 text-primary font-semibold before:bg-primary' : 'text-soft hover:text-primary hover:bg-primary/10 before:bg-primary/60'} transition-colors`}
                          >
                            <span className={`inline-flex items-center justify-center shrink-0 h-6 w-6 rounded-md text-primary ${open ? 'bg-transparent' : ''}`}>
                              <Icon size={14} />
                            </span>
  ...
  <div className="border-t border-default p-3 flex items-center gap-3 bg-surface">
        <div className="flex-1 min-w-0">
          ...
          {open && <p className="text-[10px] text-soft/70 mt-0.5">© {new Date().getFullYear()}</p>}
        </div>
        <div className="flex items-center gap-2">
          ...
          <SignedIn>
            ...
            {open && <SignOutButton className="bg-primary/10 text-primary hover:bg-primary/15" />}
          </SignedIn>
        </div>
      </div>
    </aside>
